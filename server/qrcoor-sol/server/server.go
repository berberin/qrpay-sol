	...
	if message.Type == formation.MessageTypeConnect {
		...
		case formation.RoleTypePayer:
			...
			for {
				...
				case formation.MessageTypeReception:
					...
					connection.AddTransaction("OK")
					billConn.Payer[connectMessage.Address] = connection

					// wait to send to
					if librarian.AllTransactionIsSet(connectMessage.BillID) && !librarian.TransactionsSent[bill.ID] {
						librarian.TransactionsSent[bill.ID] = true
						// send to issuer
