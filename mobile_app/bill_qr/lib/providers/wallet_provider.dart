...
class WalletProvider {
  ...
  static Future<String> createNewWallet() async {
    ...
    Providers.jsContext.evalJavascript("""      
        nativeCommunicator.postMessage(JSON.stringify({tag: '$tag', data: solana.createAccount()}));
    """);
  ...
  static getAddress() async {
    String tag = randomAlpha(5);
    Providers.jsContext.evalJavascript("""
        nativeCommunicator.postMessage(JSON.stringify({tag: '$tag', data: solana.getPublickey('$privateKey')}));
    """);
    dynamic res = await Providers.waitJSMessage(tag);
    return res.toString();
  }
  ...
  static Future<String> createTransaction(
    ...
    String tag = randomAlpha(5);
    Providers.jsContext.evalJavascript("""
      solana.createFeeBumpTx('$privateKey', '$toAddress', $amount)
      .then(
        value => {
          nativeCommunicator.postMessage(JSON.stringify({tag: '$tag', data: value}));
        }
  ...
  static Future<dynamic> createFeebump(String objTx) async {
    ...
    String tag = randomAlpha(5);
    Providers.jsContext.evalJavascript("""
      solana.submitFeeBumpTrx('$privateKey', '$objTx')
      .then(
        value => {
          nativeCommunicator.postMessage(JSON.stringify({tag: '$tag', data: value}));
       }
  ...
  static transfer(String privateKey, String toAddress, double parse) async {
    String tag = randomAlpha(5);
    Providers.jsContext.evalJavascript("""
      solana.transfer('$privateKey', '$toAddress' ,'$parse')
      .then(
        value => {
          nativeCommunicator.postMessage(JSON.stringify({tag: '$tag', data: value}));
       }
